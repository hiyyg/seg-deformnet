...
class PoseDataset(data.Dataset):
    def __init__(self, source, mode, data_dir, n_pts, img_size, rotate_to_center=True):
        """
        Args:
            source: 'CAMERA', 'Real'
            mode: 'train' or 'test'
    ...
    def __getitem__(self, index):
        ...
        choose_depth = (np.floor(row_idx * ratio) * self.img_size + np.floor(col_idx * ratio)).astype(np.int64)
