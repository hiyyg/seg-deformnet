...
class PoseDataset(data.Dataset):
    def __init__(self, source, mode, data_dir, n_pts, beta_train, beta_test, rotate_to_center=True):
        """
        Args:
            source: 'CAMERA', 'Real'
            mode: 'train' or 'test'
        ...
        data_list_path = ['CAMERA/train_list.txt', 'Real/train_list{}.txt'.format(beta_train),
                         'CAMERA/val_list.txt', 'Real/test_list{}.txt'.format(beta_test)]
        if mode == 'train':
    ...
    def __getitem__(self, index):
        ...
        if 'scene' in data_parsing[0]:
            ...
            if self.mode == 'train':
                label_suffix = '_label{}.pkl'.format(self.beta_train)
                mask_suffix = '_adjusted_mask{}.png'.format(self.beta_train)
            elif self.mode == 'test':
                label_suffix = '_label{}.pkl'.format(self.beta_test)
                mask_suffix = '_adjusted_mask{}.png'.format(self.beta_test)
            with open(img_path + label_suffix, 'rb') as f:
                gts = cPickle.load(f)
            mask = cv2.imread(img_path + mask_suffix)[:, :, 2]  
