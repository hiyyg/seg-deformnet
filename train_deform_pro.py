...
from data.pose_dataset_pro import PoseDataset
...
parser.add_argument('--max_epoch', type=int, default=60, help='max number of epochs to train')
...
parser.add_argument('--val_size', type=int, default=2500, help='val size for a epoch')
parser.add_argument('--points_process', type=str, default='', help='combation of fru, NP, SC')
opt = parser.parse_args()

# opt.dataset = 'CAMERA+Real'
# opt.max_epoch = 50
# opt.result_dir = 'results/Real/real_lrNP_pc50_vs2500'
# opt.points_process = 'NP'
visflag = 0

if opt.dataset == 'CAMERA':
    opt.decay_epoch = [0, 12, 24, 36, 48]
else:
...
def train_net():
    ...
    # dataset
    train_dataset = PoseDataset(opt.dataset, 'train', opt.data_dir, opt.n_pts, opt.img_size, opt.points_process, vis=visflag)
    val_dataset = PoseDataset(opt.dataset, 'test', opt.data_dir, opt.n_pts, opt.img_size, opt.points_process, vis=visflag)
    # start training
    st_time = time.time()
    train_steps = 1500
    ...
    for epoch in range(opt.start_epoch, opt.max_epoch + 1):
        ...
        for i, data in enumerate(train_dataloader, 1):
            ...
            sRT = sRT.cuda()
            nocs = nocs.cuda()
            assign_mat, deltas = estimator(points_pro, rgb, choose, cat_id, prior)
